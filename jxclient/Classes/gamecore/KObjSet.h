...
typedef struct
{
	...
	int     m_ItemLevel;
	int     m_StackNum;
	//DWORD   m_cAttackerDwid;  //上次攻击着的DWID
} KCObjItemInfo;
...
public:
	...
	int			m_nObjID;
	//unsigned long	m_dwNameColor[MAX_OBJ_NAME_COLOR];  // 物品名字颜色数组
	ccColor3B	m_dwNameColor[MAX_OBJ_NAME_COLOR];
...
//	int		AddMoneyObj(KMapPos MapPos, int nMoneyNum);
	...
	int		ClientAdd(int nID, int nDataID, int nState, int nDir, int nCurFrame, int nXpos, int nYpos, KCObjItemInfo sInfo,unsigned long nObjDwidx=0);
	BOOL	ClientLoadRegionObj(char *lpszMapPath, int nRegionX, int nRegionY, int nSubWorld, int nRegion);
	BOOL	ClientAddRegionObj(KPakFile *pFile, unsigned long dwDataSize);
	void	RemoveIfClientOnly(int nIdx);
	int		SearchObjAt(int nX, int nY, int nRange);
	ccColor3B	GetNameColor(int nColorID);
