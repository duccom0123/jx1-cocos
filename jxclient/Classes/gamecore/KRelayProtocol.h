...
{
	UINT	nToIP;									//255.255.255.255是广播, 0.0.0.0是目的地已经到达
	UINT	nToRelayID;								//0是网内广播, -1是网外广播
	UINT	nFromIP;								//0.0.0.0时表明刚进入Relay体系，Relay会填写nFromIP和nFromRelayID
	UINT	nFromRelayID;
...
struct RELAY_ASKWAY_DATA : EXTEND_HEADER
{
	UINT	nFromIP;								//0.0.0.0时表明刚进入Relay体系，Relay会填写nFromIP和nFromRelayID
	UINT	nFromRelayID;
	UINT	seekRelayCount;							//查找经历的Relay的个数，到-1时为不可达
