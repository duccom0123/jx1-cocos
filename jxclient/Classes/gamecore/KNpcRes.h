...
private:
	...
	int             m_nPifengType;                      //披风类型
	int             m_nChiBangType;                      //翅膀类型
...
public:
	enum
	{
			...
			adjustcolor_poison,         // 毒素伤害
			adjustcolor_freeze,			// 冰冻伤害
			adjustcolor_burn,			// 火焰伤害
			adjustcolor_confuse,		// 闪电伤害
			adjustcolor_stun,			// 眩晕伤害颜色
			adjustcolor_red=16,			// 红色
	};
	KSprControl		m_cNpcImage[MAX_PART];				 // 所有动作的所有spr文件名
	KSprControl		m_cNpcShadow;					     // npc阴影
	KStateSpr		m_cStateSpr[36];                    //特效，0 1 2 3 为头顶 4 5 6 7 为脚底  8 9 10 11为身上
	//KStateSpr		m_ClientStateSpr[20];               //客户端最大同步的数量
	//KRUImage		m_ClientDrawFile[20];               //客户端状态结构
	//KSprControl	m_cSpecialSpr;						// 特殊的只播放一遍的随身spr文件
	//KSprControl	m_cFrameSpr;
	unsigned int	m_ulAdjustColorId;   //状态颜色偏移表ID
	...
	//KRUImage		m_cStatrDrawFile[MAX_NPC_IMAGE_NUM];     // 绘制列表 身体部件 + 阴影 + 魔法状态 + 特殊动画 + 头顶状态
...
public:
	KNpcRes();
	~KNpcRes();
	BOOL			Init(char *lpszNpcName, KNpcResList *pNpcResList,unsigned int nNpcDwIdx);	// 初始化
...
//	void	        SetClientState(KList *pNpcStateList, KNpcResList *pNpcResList);
	...
	void			SetFrameSpr(char *lpszSprName, int nX, int nY, int nHeight,int mInterval=100); //设置动态循环SPR
