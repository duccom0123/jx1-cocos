...
#define	MAX_BODY_PART			6		// 整个npc分为几个大部分  5
#define	MAX_BODY_PART_SECT		4		// 每个大部分分为几个小部分
#define	MAX_PART				(MAX_BODY_PART*MAX_BODY_PART_SECT) //24
#define MAX_NPC_IMAGE_NUM		(1 + MAX_PART + 12 + 1 + 1)        //绘画的数量(可重复使用) 绘制列表 阴影+ 身体部件  + 魔法状态 + 特殊动画(一次性) + 头顶状态(循环SPR)
#define	MAX_ACTION				128  //行为数量限制
#define MAX_RES_STATE			500  //状态的数量限制
...
{
private:
	int			m_nType[MAX_RES_STATE];			// 类型：头顶、脚底、身上   位置
	...
	int			m_nMapPosX[MAX_RES_STATE];      // spr 文件帧间隔
	int			m_nMapPosY[MAX_RES_STATE];      // spr 文件帧间隔
	char		m_szName[MAX_RES_STATE][128];	// spr 文件名
public:
