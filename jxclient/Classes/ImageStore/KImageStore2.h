...
//====针对单个图形的操作====
	...
	*/
    ...
    void FreeImage(
        //## 图形文件名/图形名。
        char* pszImage);
	void* GetSprFrameData(char* pszImage, unsigned int& uImage,
    ...
    void* GetImage(
		...
		void*& pFrameData,
		void*& pOffsetTable,
		int& bSingleFrameLoad,unsigned int &nSingFrameSize);
...
private:
	...
	void* GetSprFrame(
		...
		int nFrame,unsigned int& nSingFrameSize);
    ...
    void* LoadImage(
		...
		int nFrame,
		//用于获取帧数据的缓冲区指针
		void*& pFrameData,unsigned int& nSingFrameSize);

	//文件名转化为字符串
	unsigned int ImageNameToId(char* pszName);
