...
{   
	...
	if (root==NULL)
	{
	//	printf("--[查找到节点数:root=NULL] -- \n");
		return NULL;
	}

    TreeNode<T> *t = root;	//从根节点开始找
    ...
    while(t != NULL)
    {
        ...
        else 
        {
            // 修改双亲指针，并移到左子树或右子树
			//printf("--[查找到节点数:%d] -- \n",i);
            parent = t;		 //将上个节点 赋值给双亲节点（相对上节点而言 是双亲节点）
...
bool BinSTree<T>::Insert(const T& item)
{
	...
	{
	    TreeNode<T> *newNode = new TreeNode<T>(item,NULL,NULL);	 //创建很多棵树？
...
{//递归方法
	...
	if(t->left)
		i=BiTreeDepth(t->left);   // 左子树深度
	else
		i=0;

	if(t->right)
		j=BiTreeDepth(t->right);  // 右子树深度
	else
		j=0;
...
{ // Base Cases: 空树或者值为根节点值 
	...
	// 指定值大于根值 
	if (root->key < key)
		return search(root->right, key); // 指定值小于根植 
 ...
 第二层有2个节点，这是2^1
 。。。。。。
第n层有2^(n-1)个结点，然后根据等比数列求和公式，能计算出总的结点的数目为2^n - 1，
所以当高度是n的时候，总结点的个数是2^n - 1，然后利用对数，就算出来高度和总结点数的关系了
*/
