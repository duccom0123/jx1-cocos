...
SPRHEAD* SprGetHeader(char* pszFileName, SPROFFS*& pOffsetTable)
{
	...
	KPakFile	File;
	if (!File.Open(pszFileName))  //打开SPR文件
	{
		//char springo[128]={0};
	...
	SPRHEAD*		pSpr = NULL;
	if (File.IsFileInPak())      //判断此文件是否从包中打开的
	{
		//====到文件包内寻找读取图文件=====
	...
	else
	{
		...
		{
			if (*(int*)&Header.Comment[0] != SPR_COMMENT_FLAG || Header.Colors > 256)  //原来有的
				break;
			//---为输出缓冲区分配空间---
...
BOOL KPakFile::Open(char* pszFileName)
{
	...
	{
		bOk = (m_File.Open((char*)pszFileName) != FALSE); //打开文件（支持所有后缀名）(服务器端直接读取文件)
		if (bOk == false && g_pPakList)  //读包文件
		{//查找子文件
	...
	{
		if (g_pPakList) //读取PAK包的  //赋值给包头文件结构
			bOk = g_pPakList->pGetFilePath(pszFileName,m_PackRef);
		if (bOk == false)//如果包里没有,就读取 文件包外的 取消读包外的文件
...
//#ifndef _SERVER
	if (m_PackRef.nPackIndex >= 0)  //包中  INI文件
	{
		//MessageBox(NULL,"包中存在文件","Information",MB_OK | MB_ICONEXCLAMATION);

		if (g_pPakList->ElemFileRead(m_PackRef, pBuffer, uSize) == false)
		{
			//MessageBox(NULL,"找不到包文件","Information",MB_OK | MB_ICONEXCLAMATION);
			uSize = 0;
		}
	}
	else
