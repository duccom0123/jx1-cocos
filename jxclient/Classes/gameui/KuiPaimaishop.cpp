...
//创建确定弹出框
KuiPaimaishop * KuiPaimaishop::create(KUiPlayerItem* pDest,CCObject * callbackListener,SEL_CallFuncN callfun)
{
...
bool KuiPaimaishop::init()
{
	...
	t_sprintf(nSprName,"\\spr\\Ui3\\买卖\\新奇珍阁界面\\拍卖行背景.spr");
	g_StrLower(nSprName);
...
void KuiPaimaishop::setcoloseButton(CCObject * callbackListener,SEL_CallFuncN callfun)
{
	...
	CCMenuItemSprite * buyConfirm   = CCMenuItemSprite::create(sprite_buy_normal, sprite_buy_select, this, menu_selector(KuiPaimaishop::buyCallBackFunc));
	...
	CCMenuItemSprite * closeConfirm   = CCMenuItemSprite::create(sprite_close_normal, sprite_close_select, this, menu_selector(KuiPaimaishop::closeCallBackFunc));
	...
	CCMenuItemSprite * backConfirm   = CCMenuItemSprite::create(sprite_back_normal, sprite_back_select, this, menu_selector(KuiPaimaishop::backCallBackFunc));
	...
	CCMenuItemSprite * nextConfirm   = CCMenuItemSprite::create(sprite_next_normal, sprite_next_select, this, menu_selector(KuiPaimaishop::nextCallBackFunc));
...
//上一页
void KuiPaimaishop::backCallBackFunc(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{
		m_descPadIsOpen =false;
...
//下一页
void KuiPaimaishop::nextCallBackFunc(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{
		m_descPadIsOpen =false;
...
//执行上层对象的回调函数，关闭弹出框
void KuiPaimaishop::buyCallBackFunc(CCObject * pSender)
{
	if  (m_selItemIndex<0)
	{
#ifdef WIN32
...
void KuiPaimaishop::closeCallBackFunc(CCObject * pSender)
{
...
//关闭弹出框
void KuiPaimaishop::closePopLayer(CCObject * pSender)
{
	this->removeFromParentAndCleanup(true);
}
...
CCRect KuiPaimaishop::getRect(CCNode* pNode,int i)
{
	CCRect  rc;
...
bool KuiPaimaishop::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
		...
		if (m_ItemIndex[i]>=0)
		{
			 ...
			 if  (nItemSpr && ncolorlable)
			 {
				 ...
				 {//点中了
					...
					// CCLayer * sDescLayer = KuiItemdesc::create("",second,this,callfuncN_selector(KuiPaimaishop::closedescPadCallback));
					 CCLayer * sDescLayer = KuiItemdesc::create(m_CurShopIdx,second,this,callfuncN_selector(KuiPaimaishop::closedescPadCallback));
...
void KuiPaimaishop::closedescPadCallback(CCNode *pNode)
{
	m_descPadIsOpen = false;
}
