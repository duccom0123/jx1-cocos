...
//创建确定弹出框
KuiDynamicShop * KuiDynamicShop::create(BuySellInfo* pBSinfo,CCObject * callbackListener,SEL_CallFuncN callfun)
{
...
bool KuiDynamicShop::init()
{
	...
	t_sprintf(nSprName,"\\Spr\\Ui3\\买卖\\商店界面\\精炼石商店.spr");
	g_StrLower(nSprName);
...
void KuiDynamicShop::setcoloseButton(CCObject * callbackListener,SEL_CallFuncN callfun)
{
	...
	ZeroMemory(&nSprInfo,sizeof(nSprInfo));
	ZeroMemory(nSprName,sizeof(nSprName));
	t_sprintf(nSprName,"\\Spr\\Ui3\\买卖\\商店界面\\商店标题.spr");
	g_StrLower(nSprName);
	...
	titleImg->setPosition(ccp(m_size.width/2,m_size.height-18));
	m_pUiLayer->addWidget(titleImg);

	ZeroMemory(nSprName,sizeof(nSprName));
	t_sprintf(nSprName,"\\Spr\\Ui3\\买卖\\商店界面\\分页按钮.spr");
	g_StrLower(nSprName);
	bgCur = _getinidata.getinidata_one(nSprName,0,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
	if (bgCur==NULL)
		return ;
	...
	for (int i = 0; i < nTypeCount; i ++)	 //0 1 2 3 =2 3 4 5
	{
		...
		//m_pSelf->m_SellType[i].SetLabel(szName);	//商城类型
		cTabFile.GetInteger(i+2, "shopid", -1,&nShopIdx);
		m_nCurrentShopId[i] = nShopIdx;    //商店Idx

		btnPageBox[i] = UIButton::create();
		...
		if (i==0)
		    btnPageBox[i]->setPosition(ccp(36+check_btn_normal->getContentSize().width*i,m_size.height-nOffsetY));
		else
		{
			if  (i>=6)
			{
			   ...
			   if (i==6)
				   btnPageBox[i]->setPosition(ccp(36+check_btn_normal->getContentSize().width*(i-6),m_size.height-nOffsetY));
			   else
				   btnPageBox[i]->setPosition(ccp((36+check_btn_normal->getContentSize().width+2)*(i-6),m_size.height-nOffsetY));
			}
			else
			   btnPageBox[i]->setPosition(ccp(36+(check_btn_normal->getContentSize().width+2)*i,m_size.height-nOffsetY));
		}
		btnPageBox[i]->setTag(i+1);
		//btnPageBox[i]->setTagbyKey("1");
		//btnPageBox_1->ignoreContentAdaptWithSize(true);
		btnPageBox[i]->addTouchEventListener(this,toucheventselector(KuiDynamicShop::touchPageBtnEvent));
		m_pUiLayer->addWidget(btnPageBox[i]);
		//if (!szName[0])
	...
	CCMenuItemSprite * buyConfirm   = CCMenuItemSprite::create(sprite_buy_normal, sprite_buy_select, this, menu_selector(KuiDynamicShop::buyCallBackFunc));
	...
	*/
	ZeroMemory(nSprName,sizeof(nSprName));
	t_sprintf(nSprName,"\\Spr\\Ui3\\买卖\\商店界面\\关闭.spr");
	g_StrLower(nSprName);
	ZeroMemory(&nSprInfo,sizeof(nSprInfo));
	bgCur = _getinidata.getinidata_one(nSprName,0,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
	if (bgCur==NULL)
	...
	CCMenuItemSprite * closeConfirm   = CCMenuItemSprite::create(sprite_close_normal, sprite_close_select, this, menu_selector(KuiDynamicShop::closeCallBackFunc));
	closeConfirm->setPosition(ccp(0,-30));
	...
	CCMenuItemSprite * backConfirm   = CCMenuItemSprite::create(sprite_back_normal, sprite_back_select, this, menu_selector(KuiDynamicShop::backCallBackFunc));
	backConfirm->setPosition(ccp(65,6));
	...
	CCMenuItemSprite * nextConfirm   = CCMenuItemSprite::create(sprite_next_normal, sprite_next_select, this, menu_selector(KuiDynamicShop::nextCallBackFunc));
	nextConfirm->setPosition(ccp(155,6));
...
void KuiDynamicShop::touchPageBtnEvent(CCObject *pSender, TouchEventType type)
{
...
//上一页
void KuiDynamicShop::backCallBackFunc(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{
		m_descPadIsOpen =false;
...
//下一页
void KuiDynamicShop::nextCallBackFunc(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{
		m_descPadIsOpen =false;
...
//执行上层对象的回调函数，关闭弹出框
void KuiDynamicShop::buyCallBackFunc(CCObject * pSender)
{
	if  (m_selItemIndex<0)
	{
#ifdef WIN32
...
void KuiDynamicShop::closeCallBackFunc(CCObject * pSender)
{
...
//关闭弹出框
void KuiDynamicShop::closePopLayer(CCObject * pSender)
{
	this->removeFromParentAndCleanup(true);
}
...
CCRect KuiDynamicShop::getRect(CCNode* pNode,int i)
{
	CCRect  rc;
...
bool KuiDynamicShop::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
		...
		if (m_ItemIndex[i]>=0)
		{
			 ...
			 if  (nItemSpr && ncolorlable)
			 {
				 ...
				 {//点中了
					...
					// CCLayer * sDescLayer = KuiItemdesc::create("",second,this,callfuncN_selector(KuiDynamicShop::closedescPadCallback));
					 CCLayer * sDescLayer = KuiItemdesc::create(m_CurShopIdx,second,this,callfuncN_selector(KuiDynamicShop::closedescPadCallback));
...
void KuiDynamicShop::closedescPadCallback(CCNode *pNode)
{
	m_descPadIsOpen = false;
}
...
void KuiDynamicShop::SellSortChange(int i)
{
	//m_nCurrentShopId = i;
	if (g_pCoreShell)
		g_pCoreShell->OperationRequest(GOI_SYN_SHOPIDX,0,m_nCurrentShopId[i]); //同步了ID到服务器
...
void KuiDynamicShop::UpdateData(int nSellIdx)
{
	...
	if (m_pObjsList = (KUiObjAtContRegion*)malloc(sizeof(KUiObjAtContRegion) * m_nObjCount))
	{
		...
		g_pCoreShell->GetDataDynamicShop(m_nCurrentShopId[nSellIdx], (unsigned int)m_pObjsList, m_nObjCount);//单线程执行，nCount值不变
