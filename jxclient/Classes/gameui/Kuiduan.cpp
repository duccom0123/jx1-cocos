...
//创建确定弹出框
Kuiduan * Kuiduan::create(char * strTitel,char *pContent,int nNum,char* pszCallback,char* pszError,CCObject * callbackListener,SEL_CallFuncN callfun)
{
...
void Kuiduan::setcoloseButton(CCObject * callbackListener,SEL_CallFuncN callfun)
{
	...
	CCMenuItemSprite * okConfirm = CCMenuItemSprite::create(sprite_ok_btn_confirm_normal, sprite_ok_btn_confirm_select, this, menu_selector(Kuiduan::okCallBackFunc));
	...
	CCMenuItemSprite * closeConfirm = CCMenuItemSprite::create(sprite_close_normal, sprite_close_select, this, menu_selector(Kuiduan::buttonCallBackFunc));
	...
	CCMenuItemSprite * fanzhiConfirm = CCMenuItemSprite::create(sprite_fanzhi_normal, sprite_fanzhi_select, this, menu_selector(Kuiduan::btnCallBackFunc_fangzhi));
	...
	CCMenuItemSprite * quxiaConfirm = CCMenuItemSprite::create(sprite_quxia_normal, sprite_quxia_select, this, menu_selector(Kuiduan::btnCallBackFunc_quxia));
	...
	CCMenuItemSprite * zhengliConfirm = CCMenuItemSprite::create(sprite_zhengli_normal, sprite_zhengli_select, this, menu_selector(Kuiduan::zhengCallBackFunc));
...
void Kuiduan::zhengCallBackFunc(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{  
	}
...
void Kuiduan::okCallBackFunc(CCObject * pSender)
{
	if (g_pCoreShell)
	{
...
void Kuiduan::btnCallBackFunc_fangzhi(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{  
		m_descPadIsOpen = false;
...
void Kuiduan::btnCallBackFunc_quxia(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{  
	}
...
//执行上层对象的回调函数，关闭弹出框
void Kuiduan::buttonCallBackFunc(CCObject * pSender)
{
...
//关闭弹出框
void Kuiduan::closePopLayer(CCObject * pSender)
{
	this->removeFromParentAndCleanup(true);
}
...
void Kuiduan::closeItemPadCallback(CCNode *pNode)
{//在这个层中标记号，以确定是哪个按钮
	m_descPadIsOpen = false;
}

CCRect Kuiduan::getGiveItem(CCNode* pNode,int i)
{
		/*CCRect  rc;
...
CCRect Kuiduan::getRectItem(CCNode* pNode,int i)
{
	CCRect  rc;
	rc.origin     = pNode->getPosition(); //这个因该是图内偏移
	rc.size       = pNode->getContentSize();
...
bool Kuiduan::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
			...
			if  (nCurColorlayer)
			{
				...
				{//点中了
					if (SkillNode)
					{
						...
						CCLayer * sDescLayer = KuiItemdesc::create(it->second,this,callfuncN_selector(Kuiduan::closeItemPadCallback),1);
			...
			if  (nCurColorlayer)
			{
				...
				{//点中了
					...
					if (SkillNode)
					{
						...
						CCLayer * sDescLayer = KuiItemdesc::create(its->second,this,callfuncN_selector(Kuiduan::closeItemPadCallback),1);
						m_descPadIsOpen = true;
						/*char msg[64];
