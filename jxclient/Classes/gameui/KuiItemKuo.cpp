...
//创建确定弹出框
KuiItemKuo * KuiItemKuo::create(CCObject * callbackListener,SEL_CallFuncN callfun)
{
...
bool KuiItemKuo::init()
{
	...
	*/
	...
	t_sprintf(nSprName,"\\Spr\\Ui3\\增加储物箱\\储藏箱增加.spr");
	g_StrLower(nSprName);
	...
	spriteBox->setPosition(ccp(winSize.width/2-30,winSize.height/2));
...
void KuiItemKuo::setcoloseButton(CCObject * callbackListener,SEL_CallFuncN callfun)
{
	m_callbackListener = callbackListener;
	m_callfun = callfun;
	//\Spr\Ui3\增加储物箱\关闭.spr
	char nSprName[128]={0};
	ZeroMemory(nSprName,sizeof(nSprName));
	t_sprintf(nSprName,"\\Spr\\Ui3\\增加储物箱\\关闭.spr");
	g_StrLower(nSprName);
	int m_nWidth,m_nHeight,nFrams;
	CCTexture2D *bgCur = NULL;
	SPRFRAMSINFO nSprInfo;
	ZeroMemory(&nSprInfo,sizeof(nSprInfo));
	CCSprite * sprite_colse_normal;
	CCSprite * sprite_colse_select;
	bgCur = _getinidata.getinidata_one(nSprName,0,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
	if (bgCur==NULL)
		return;
	sprite_colse_normal = CCSprite::createWithTexture(bgCur);

	bgCur = _getinidata.getinidata_one(nSprName,1,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
	if (bgCur==NULL)
		return;
	sprite_colse_select = CCSprite::createWithTexture(bgCur);//CCSprite::create("close.png");//CCSprite::createWithSpriteFrame(f_tips_btn_confirm_select);
	CCMenuItemSprite * closeConfirm = CCMenuItemSprite::create(sprite_colse_normal, sprite_colse_select, this, menu_selector(KuiItemKuo::buttonCallBackFunc));
	closeConfirm->setPosition(ccp(-5,-23));
	
	ZeroMemory(nSprName,sizeof(nSprName));
	t_sprintf(nSprName,"\\Spr\\Ui3\\增加储物箱\\一.spr");
	g_StrLower(nSprName);
	bgCur = _getinidata.getinidata_one(nSprName,0,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
	if (bgCur==NULL)
		return;
	...
	if (bgCur==NULL)
		return;
	sprite_colse_select = CCSprite::createWithTexture(bgCur);//CCSprite::create("close.png");//CCSprite::createWithSpriteFrame(f_tips_btn_confirm_select);
	CCMenuItemSprite * kuoConfirm_1 = CCMenuItemSprite::create(sprite_colse_normal, sprite_colse_select, this, menu_selector(KuiItemKuo::buttonCallBackFunc_1));
	//kuoConfirm_1->setScaleX(2);
	kuoConfirm_1->setScale(2);
	kuoConfirm_1->setPosition(ccp(-m_size.width/2+45,m_size.height-92));

	ZeroMemory(nSprName,sizeof(nSprName));
	t_sprintf(nSprName,"\\Spr\\Ui3\\增加储物箱\\二.spr");
	g_StrLower(nSprName);
	bgCur = _getinidata.getinidata_one(nSprName,0,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
	if (bgCur==NULL)
		return;
	...
	if (bgCur==NULL)
		return;
	sprite_colse_select = CCSprite::createWithTexture(bgCur);//CCSprite::create("close.png");//CCSprite::createWithSpriteFrame(f_tips_btn_confirm_select);
	CCMenuItemSprite * kuoConfirm_2 = CCMenuItemSprite::create(sprite_colse_normal, sprite_colse_select, this, menu_selector(KuiItemKuo::buttonCallBackFunc_2));
	kuoConfirm_2->setScale(2);
	kuoConfirm_2->setPosition(ccp(-m_size.width/2+97,m_size.height-92));

	ZeroMemory(nSprName,sizeof(nSprName));
	t_sprintf(nSprName,"\\Spr\\Ui3\\增加储物箱\\三.spr");
	g_StrLower(nSprName);
	bgCur = _getinidata.getinidata_one(nSprName,0,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
	if (bgCur==NULL)
		return;
	...
	if (bgCur==NULL)
		return;
	sprite_colse_select = CCSprite::createWithTexture(bgCur);//CCSprite::create("close.png");//CCSprite::createWithSpriteFrame(f_tips_btn_confirm_select);
	CCMenuItemSprite * kuoConfirm_3 = CCMenuItemSprite::create(sprite_colse_normal, sprite_colse_select, this, menu_selector(KuiItemKuo::buttonCallBackFunc_3));
	kuoConfirm_3->setScale(2);
	kuoConfirm_3->setPosition(ccp(-m_size.width/2+150,m_size.height-92));
	...
	CCMenuItemSprite * quConfirm = CCMenuItemSprite::create(sprite_qu_normal, sprite_qu_select, this, menu_selector(KuiItemKuo::quCallBackFunc));
	...
	CCMenuItemSprite * qiConfirm = CCMenuItemSprite::create(sprite_qi_normal, sprite_qi_select, this, menu_selector(KuiItemKuo::qiCallBackFunc));
	...
	CCMenuItemSprite * zenConfirm = CCMenuItemSprite::create(sprite_zheng_normal, sprite_zheng_select, this, menu_selector(KuiItemKuo::zenliCallBackFunc));
	zenConfirm->setPosition(ccp(m_size.width/2+20,m_size.height/2-sprite_zheng_select->getContentSize().height*2-10*2));
	
	CCMenu * menu = CCMenu::create(closeConfirm,kuoConfirm_1,kuoConfirm_2,kuoConfirm_3,quConfirm,qiConfirm,zenConfirm,NULL);
	menu->setPosition(ccp(m_size.width/2,45));//菜单位置设置为弹出框左下
	ParentNode_ItemEx->addChild(menu);         //加到父层里
}

void KuiItemKuo::zenliCallBackFunc(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{  
		m_descPadIsOpen = false;
	...
	{
#ifdef WIN32
		CCMessageBox("扩展箱还没有开启","提示:");
#else
		CCMessageBox(UTEXT("扩展箱还没有开启",1).c_str(),UTEXT("提示:",1).c_str());
#endif	
		return;
	}
...
void KuiItemKuo::qiCallBackFunc(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{  
		m_descPadIsOpen = false;
...
void KuiItemKuo::quCallBackFunc(CCObject * pSender)
{
...
void KuiItemKuo::buttonCallBackFunc_1(CCObject * pSender)
{
	UpdateData(GDI_ITEM_IN_EX_BOX1);
}
void KuiItemKuo::buttonCallBackFunc_2(CCObject * pSender)
{
	UpdateData(GDI_ITEM_IN_EX_BOX2);
}
void KuiItemKuo::buttonCallBackFunc_3(CCObject * pSender)
{
	UpdateData(GDI_ITEM_IN_EX_BOX3);
}
//执行上层对象的回调函数，关闭弹出框
void KuiItemKuo::buttonCallBackFunc(CCObject * pSender)
{
	CCNode * node = NULL;
	if (pSender)
...
//关闭弹出框
void KuiItemKuo::closePopLayer(CCObject * pSender)
{
	this->removeFromParentAndCleanup(true);
}
...
CCRect KuiItemKuo::getRect(CCNode* pNode,int i)
{
	CCRect  rc;
	rc.origin     = pNode->getPosition();          //这个因该是图内偏移
	//visibleSize
	rc.size       = pNode->getContentSize();
	rc.origin.x   = m_origin.x + rc.origin.x + 15; //起点最左边 - m_size.width/2 
	rc.origin.y   = m_origin.y - m_size.height/2 + rc.origin.y+10; //最上边    //- m_size.height/2
	
	return rc;
}

void KuiItemKuo::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent)
{
...
//移动中
void KuiItemKuo::ccTouchMoved(CCTouch *pTouch, CCEvent *pEvent)
{
	if (!m_bScrolling) return;
...
bool KuiItemKuo::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
	...
	if  (!m_nIstrue)
	{
		nstrCurSelItemKey ="";
#ifdef WIN32
		CCMessageBox("扩展箱没有开启","提示");
#else
		CCMessageBox(UTEXT("扩展箱没有开启",1).c_str(),UTEXT("提示",1).c_str());
#endif
		return true;
	}
			...
			if  (nCurColorlayer)
			{
				...
				{//点中了
					if (SkillNode)
					{
						...
						CCLayer * sDescLayer = KuiItemdesc::create(it->second,this,callfuncN_selector(KuiItemKuo::closeItemPadCallback),0);
...
void KuiItemKuo::closeItemPadCallback(CCNode *pNode)
{//在这个层中标记号，以确定是哪个按钮
	m_descPadIsOpen = false;
}
	...
	for (int i = 0; i < nCount; ++i)
	{
			...
			if (Item && pObject->uId>0 && pObject->uId<MAX_ITEM)
			{
				...
				if  (nItemSprPath[0])
				{
					...
					m_ItemData[nKey].nGenkind = ExBoxIndex;  //随身携带
...
// m_CurSelItemIndex = -1;
  ...
  // memset(m_ItemData,0,sizeof(KUiItemdObject) * 60);
   ...
   m_ItemData.clear();
	...
	int nCount= g_pCoreShell->GetGameData(nExboxIndex, 0, 0); //得到随身带物品的数量
	...
	{
		g_pCoreShell->GetGameData(nExboxIndex, (unsigned int)pObjs, nCount);//单线程执行，nCount值不变
		for (int i = 0; i < nCount; ++i)
		{
			KUiDraggedObject no;
			...
			if (Item && pObjs[i].Obj.uId>0 && pObjs[i].Obj.uId<MAX_ITEM)
			{
					...
					if  (nExboxIndex==GDI_ITEM_IN_EX_BOX1)
					   m_ItemData[nKey].nGenkind = UOC_EX_BOX1;         //随身携带
					if  (nExboxIndex==GDI_ITEM_IN_EX_BOX2)
						m_ItemData[nKey].nGenkind = UOC_EX_BOX2;         //随身携带
					if  (nExboxIndex==GDI_ITEM_IN_EX_BOX3)
						m_ItemData[nKey].nGenkind = UOC_EX_BOX3;         //随身携带
					ccColor4B color = ccc4(112, 128, 144, 150);
					//t_sprintf(nItemKey,"color_%d",i+1);
					strcat(nItemKey,"_color");
					nKey =nItemKey;
