...
//创建确定弹出框
KuiSortSkill_r * KuiSortSkill_r::create(int nKind,CCObject * callbackListener,SEL_CallFuncN callfun)
{
...
void KuiSortSkill_r::addDialogData(int nKind)
{
	...
	*/
	...
	memset(m_Skills,0,sizeof(KUiSkillData) * SKILLTREE_MAX_SKILL_COUNT);
	if (g_pCoreShell)
	   _sortCount_r	=g_pCoreShell->GetGameData(GDI_RIGHT_ENABLE_SKILLS, (unsigned int)m_Skills, 0); //获取分页技能数据
	...
	{//开始添加 按钮
		...
		for (int i=0;i<_sortCount_r;i++)
		{
			...
			{//绘画技能图标
			  ...
			  CCMenuItemSprite *tempSpr=CCMenuItemSprite::create(btn_sprite,btn_sprite,NULL,this,menu_selector(KuiSortSkill_r::skillCallback));
...
//菜单回调
void KuiSortSkill_r::skillCallback(CCObject* pSender)
{
	...
	if (pSender)
	{
		...
		if (nCerBox)
		{
		  ...
		  {//绘画技能图标
			...
			CCLayer * sDescLayer = KuiSkilldesc::create(m_Skills[nSkillIdx].uId,nCurLevel,skillData,this,callfuncN_selector(KuiSortSkill_r::closeSkilldescPadCallback),2);
...
void KuiSortSkill_r::closeSkilldescPadCallback(CCNode *pNode)
{
...
void KuiSortSkill_r::setcoloseButton(CCObject * callbackListener,SEL_CallFuncN callfun)
{
	m_callbackListener = callbackListener;
	m_callfun = callfun;
}
//执行上层对象的回调函数，关闭弹出框
void KuiSortSkill_r::buttonCallBackFunc(CCObject * pSender)
{
	CCNode * node = NULL;
	if (pSender)
...
//关闭弹出框
void KuiSortSkill_r::closePopLayer(CCObject * pSender)
{
	this->removeFromParentAndCleanup(true);
}
...
CCRect KuiSortSkill_r::getRect(CCNode* pNode,int i)
{
	CCRect  rc;
...
bool KuiSortSkill_r::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
