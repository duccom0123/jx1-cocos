...
//创建确定弹出框
KuiSuperShop * KuiSuperShop::create(BuySellInfo* pBSinfo,CCObject * callbackListener,SEL_CallFuncN callfun)
{
...
bool KuiSuperShop::init()
{
	...
	t_sprintf(nSprName,"\\spr\\Ui3\\买卖\\新奇珍阁界面\\奇珍阁背景.spr");
	g_StrLower(nSprName);
...
void KuiSuperShop::setcoloseButton(CCObject * callbackListener,SEL_CallFuncN callfun)
{
	...
	CCMenuItemSprite * buyConfirm   = CCMenuItemSprite::create(sprite_buy_normal, sprite_buy_select, this, menu_selector(KuiSuperShop::buyCallBackFunc));
	...
	ZeroMemory(nSprName,sizeof(nSprName));
	t_sprintf(nSprName,"\\spr\\Ui3\\买卖\\新奇珍阁界面\\奇珍阁关闭按键.spr");
	g_StrLower(nSprName);
	ZeroMemory(&nSprInfo,sizeof(nSprInfo));
	bgCur = _getinidata.getinidata_one(nSprName,0,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
	if (bgCur==NULL)
	...
	CCMenuItemSprite * closeConfirm   = CCMenuItemSprite::create(sprite_close_normal, sprite_close_select, this, menu_selector(KuiSuperShop::closeCallBackFunc));
	...
	CCMenuItemSprite * backConfirm   = CCMenuItemSprite::create(sprite_back_normal, sprite_back_select, this, menu_selector(KuiSuperShop::backCallBackFunc));
	...
	CCMenuItemSprite * nextConfirm   = CCMenuItemSprite::create(sprite_next_normal, sprite_next_select, this, menu_selector(KuiSuperShop::nextCallBackFunc));
...
//上一页
void KuiSuperShop::backCallBackFunc(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{
		m_descPadIsOpen =false;
...
//下一页
void KuiSuperShop::nextCallBackFunc(CCObject * pSender)
{
	if (m_descPadIsOpen)
	{
		m_descPadIsOpen =false;
...
//执行上层对象的回调函数，关闭弹出框
void KuiSuperShop::buyCallBackFunc(CCObject * pSender)
{
	if  (m_selItemIndex<0)
	{
#ifdef WIN32
...
void KuiSuperShop::closeCallBackFunc(CCObject * pSender)
{
...
//关闭弹出框
void KuiSuperShop::closePopLayer(CCObject * pSender)
{
	this->removeFromParentAndCleanup(true);
}
...
CCRect KuiSuperShop::getRect(CCNode* pNode,int i)
{
	CCRect  rc;
...
bool KuiSuperShop::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
		...
		if (m_ItemIndex[i]>=0)
		{
			 ...
			 if  (nItemSpr && ncolorlable)
			 {
				 ...
				 {//点中了
					...
					// CCLayer * sDescLayer = KuiItemdesc::create("",second,this,callfuncN_selector(KuiSuperShop::closedescPadCallback));
					 CCLayer * sDescLayer = KuiItemdesc::create(m_CurShopIdx,second,this,callfuncN_selector(KuiSuperShop::closedescPadCallback));
...
void KuiSuperShop::closedescPadCallback(CCNode *pNode)
{
	m_descPadIsOpen = false;
}
