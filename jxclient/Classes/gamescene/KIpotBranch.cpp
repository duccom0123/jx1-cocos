...
{
	CCAssert(pBranchObj && m_pFirstObject,"");

	POINT	poi, p1, p2;
	p1 = pBranchObj->oPosition;
	p2 = pBranchObj->oEndPos;

	CCAssert(p1.x != p2.x && m_oHeadPoint.x != m_oEndPoint.x,"");
...
void KIpotBranch::AddLeafPoint(KIpotLeaf* pLeaf)
{
	...
	RELATION_ENUM	eRelate = SM_Relation_PointLine(pLeaf->oPosition, m_oHeadPoint, m_oEndPoint);
		//SM_Relation_PointLine_New(pLeaf->oPosition, m_oHeadPoint, m_oEndPoint);
...
void KIpotBranch::AddPointLeafToList(KIpotLeaf*& pFirst, KIpotLeaf* pLeaf,KIpotLeaf* pParentLeaf)
{
	//_ASSERT(pLeaf);
	if (pLeaf==NULL) return;
	...
	{
		if (pL->eLeafType == KIpotLeaf::IPOTL_T_BUILDIN_OBJ &&
			(((KIpotBuildinObj*)pL)->pBio->Props & SPBIO_P_SORTMANNER_MASK) != SPBIO_P_SORTMANNER_POINT)
		{
