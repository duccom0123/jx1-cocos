...
struct KSPTrap
{
	unsigned char	cX;			//区域的X位置（范围0-16）
	unsigned char	cY;			//区域的Y位置 范围0-32）
	unsigned char	cNumCell;	//连续覆盖了几个小格子
	unsigned char	cReserved;	//保留，固定值为0
	unsigned int	uTrapId;	//根据事件关联文件名转化而得到的事件id标识(脚本ID)
};
...
struct KSPNpc
{
	...
	int					nPositionX;
	int					nPositionY;
	bool				bSpecialNpc;	// 特殊npc  蓝怪？
	char				cReserved[3];	// 保留
	char				szName[32];		// npc 名字
	short				nLevel;			// 等级
	short				nCurFrame;		// 载入时图像是第几帧
	short				nHeadImageNo;	// 头像图形编号	 NpcSettings
	short				shKind;			// npc类型
