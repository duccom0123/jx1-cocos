...
#define LOGIN_ROLE_NAME_MIN_LEN	8   //最小角色名长度限制!
#define	LOGIN_ROLE_NAME_MAX_LEN	16  //最大角色名长度限制!
...
#define	LOGIN_R_TIMEOUT						5   //账号的使用时间到期
...
struct KLoginStructHead
{
	...
	unsigned        nForbitopennum;//限制多开的数量
};
//客户端
struct KLoginAccountInfo : KLoginStructHead
{
	...
	unsigned int    nLeftTime;     //剩余时间
    unsigned int    nVipType;
	unsigned int    nExtPiont;     //剩余点数unsigned long 
    char            nBishopver[16];//版本
	unsigned int    nSelSererIdx;
